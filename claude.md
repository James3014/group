...
- **寫程式碼前先寫測試**（Test-Driven Development）
- 測試失敗 → 寫程式碼 → 測試通過 → 重構
- 沒測試過的程式碼不要提交
- 部署前必須 build 成功,零錯誤容忍
...
1. **理解需求** - 確認要解決的問題
2. **寫測試** - 先寫測試用例（如果適用）
3. **實作功能** - 寫最簡單能通過測試的程式碼
4. **執行測試** - 確保所有測試通過
5. **Build 驗證** - 執行 `npm run build` 確保零錯誤
6. **提交程式碼** - 只提交測試過的程式碼

### 測試要求

**對於前端應用:**
- 至少要能成功 build（`npm run build`）
- 手動測試核心功能流程
- 部署前必須驗證 build 成功
...
**Linus 會說:**
> "如果你連 build 都沒跑過就提交程式碼,那你是在浪費所有人的時間。測試不是可選的,是基本要求。"
